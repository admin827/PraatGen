...
2. Paste the entire contents of `MASTER_PROMPT_CORE_v14_11_0.md` into the instructions field
...
| File | Purpose |
|------|---------|
| `MASTER_PROMPT_CORE_v14_11_0.md` | The system instructions that configure Claude as a Praat scripting specialist. Contains 37 rules governing syntax validation, command verification, clinical defaults, debugging protocol, sandbox/autonomous modes, and code-quality standards. Master Prompt content version: 14.11.0. |
| `README.md` | This file. |
| `RELEASE_NOTES_1.0.3.md` | What changed in this release and the upgrade notes. Read the upgrade notes before replacing an existing installation. Also published as the body of the v1.0.3 GitHub Release. |
...
|-----------|---------|----------------|
| **Release** | 1.0.3 | The combined package (prompt + PKB). This is the version that matters to users. Tracked separately from the Master Prompt version. |
| **Master Prompt** | 14.11.0 | The system instructions. Bumped when rules, workflow, or protocols change. |
...
- **1.0.3 (30 July 2026)** is the current stable release. It ships Master Prompt 14.11.0 and folds in the fixes from the 30 July benchmark dry run. The PKB is reconciled against the EML plugin source, the procedure registry is updated from that source, and every library file is syntax-checked against Praat 6.6.30.
